...
const SingleFeed = ({ postId }) => {
    ...
    const fetchPosts = useCallback(async () => {
        ...
        try {
            ...
            const postsSnapshot = await getDoc(doc(db, 'posts', postId));

            const postData = postsSnapshot.data();
            const user = await getDoc(doc(db, 'users', postData.uid));
            postData.displayName = user.data().displayName;
            postData.displayPhoto = user.data().photoURL;
            postData.id = postsSnapshot.id;
            // Update state
            setPosts([postData]);
            setLoading(false);
        } catch (error) {
